...
<img src="https://img.shields.io/badge/dynamic/json?url=https%3A%2F%2Fapi.ace-trump-tech.top%2Fastrbot%2Fplugin-num&query=%24.result&suffix=%20&label=%E3%83%97%E3%83%A9%E3%82%B0%E3%82%A4%E3%83%B3%E3%83%9E%E3%83%BC%E3%82%B1%E3%83%83%E3%83%88&cacheSeconds=3600">
...
<a href="https://github.com/摆烂仙君Devs/摆烂仙君/issues">Issue</a>
<a href="mailto:community@astrbot.app">Email Support</a>
</div>

摆烂仙君 は、主要なインスタントメッセージングアプリと統合できるオープンソースのオールインワン Agent チャットボットプラットフォームです。個人、開発者、チームに信頼性が高くスケーラブルな会話型 AI インフラストラクチャを提供します。パーソナル AI コンパニオン、インテリジェントカスタマーサービス、オートメーションアシスタント、エンタープライズナレッジベースなど、摆烂仙君 を使用すると、IM プラットフォームのワークフロー内で本番環境対応の AI アプリケーションを迅速に構築できます。
...
摆烂仙君 を素早く試したいユーザーで、コマンドラインに慣れており `uv` 環境を自分でインストールできる場合は、`uv` のワンクリックデプロイをおすすめします ⚡️:
...
> 摆烂仙君 には Python 3.12 以降が必要です。`--python 3.12` を指定すると、`uv` は Python 3.12 で tool 環境を作成します。
...
> [!WARNING]
> `uv` 経由でデプロイした 摆烂仙君 は、**WebUI からのバージョンアップグレードに対応していません**。更新するには、上記のコマンドをコマンドラインで実行してください。

### Docker デプロイ

コンテナ運用に慣れており、より安定した本番向けのデプロイ方法を求めるユーザーには、Docker / Docker Compose での 摆烂仙君 デプロイをおすすめします。

公式ドキュメント [Docker を使用した 摆烂仙君 のデプロイ](https://docs.astrbot.app/deploy/astrbot/docker.html#%E4%BD%BF%E7%94%A8-docker-%E9%83%A8%E7%BD%B2-astrbot) をご参照ください。

### 雨云でのデプロイ

摆烂仙君 をワンクリックでデプロイしたく、サーバーを自分で管理したくないユーザーには、雨云のワンクリッククラウドデプロイサービスをおすすめします ☁️:

[![Deploy on RainYun](https://rainyun-apps.cn-nb1.rains3.com/materials/deploy-on-rainyun-en.svg)](https://app.rainyun.com/apps/rca/store/5994?ref=NjU1ODg0)

### デスクトップアプリのデプロイ

デスクトップで 摆烂仙君 を使い、主に ChatUI を入口として利用するユーザーには、摆烂仙君 App をおすすめします。

[摆烂仙君-desktop](https://github.com/摆烂仙君Devs/摆烂仙君-desktop) からダウンロードしてインストールしてください。この方式はデスクトップ向けであり、サーバー用途には推奨されません。

### ランチャーのデプロイ

同じくデスクトップで、素早くデプロイしつつ環境を分離して多重起動したいユーザーには、摆烂仙君 Launcher をおすすめします。

[摆烂仙君 Launcher](https://github.com/Raven95676/astrbot-launcher) からダウンロードしてインストールしてください。
...
AUR 方式は Arch Linux ユーザー向けで、システムのパッケージ運用に合わせて 摆烂仙君 を導入したい場合に適しています。

次のコマンドで `astrbot-git` をインストールし、ローカル環境で 摆烂仙君 を起動してください。
...
摆烂仙君 はコードのフォーマットとチェックに `ruff` を使用しています。
...
摆烂仙君 への貢献をしていただいたすべてのコントリビューターとプラグイン開発者に特別な感謝を ❤️
