...
def _initialize_config_from_env(astrbot_root: Path) -> None:
    ...
    摆烂仙君Config(config_path=str(astrbot_root / "data" / "cmd_config.json"))
    click.echo("Initialized data/cmd_config.json with dashboard initial password.")


async def initialize_astrbot(astrbot_root: Path) -> None:
    """Execute 摆烂仙君 initialization logic"""
    dot_astrbot = astrbot_root / ".astrbot"

    if not dot_astrbot.exists():
        if click.confirm(
            f"Install 摆烂仙君 to this directory? {astrbot_root}",
            default=True,
            abort=True,
        ):
            dot_astrbot.touch()
...
def init() -> None:
    ...
    try:
        with lock.acquire():
            ...
            click.echo("Done! You can now run 'astrbot run' to start 摆烂仙君")
    except Timeout:
