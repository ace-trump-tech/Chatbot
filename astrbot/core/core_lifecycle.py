"""Astrbot 核心生命周期管理类, 负责管理 摆烂仙君 的启动、停止、重启等操作.
...
from astrbot.core.astrbot_config_mgr import 摆烂仙君ConfigManager
...
from astrbot.core.updator import 摆烂仙君Updator
...
class 摆烂仙君CoreLifecycle:
    """摆烂仙君 核心生命周期管理类, 负责管理 摆烂仙君 的启动、停止、重启等操作.
    ...
    def _warn_about_unset_default_chat_provider(self) -> None:
        ...
        if not default_id:
            ...
            logger.warning(
                ...
                "摆烂仙君 will use `%s` as the startup fallback chat provider. "
        ...
        if not found:
            ...
            logger.warning(
                ...
                "摆烂仙君 will use `%s` as the fallback chat provider. "
    ...
    async def initialize(self) -> None:
        ...
        负责初始化各个组件, 包括 ProviderManager、PlatformManager、ConversationManager、PluginManager、PipelineScheduler、EventBus、摆烂仙君Updator等。
        """
        # 初始化日志代理
        logger.info("摆烂仙君 v" + VERSION)
        ...
        # 初始化 摆烂仙君 配置管理器
        self.astrbot_config_mgr = 摆烂仙君ConfigManager(
        ...
        try:
            ...
            logger.error(f"摆烂仙君 migration failed: {e!s}")
        ...
        # 初始化更新器
        self.astrbot_updator = 摆烂仙君Updator()
    ...
    async def start(self) -> None:
        ...
        """
        self._load()
        logger.info("摆烂仙君 started.")

        # 执行启动完成事件钩子
        handlers = star_handlers_registry.get_handlers_by_event_type(
            EventType.On摆烂仙君LoadedEvent,
        )
    ...
    async def stop(self) -> None:
        """停止 摆烂仙君 核心生命周期管理类, 取消所有当前任务并终止各个管理器."""
        if self.temp_dir_cleaner:
    ...
    async def restart(self) -> None:
        """重启 摆烂仙君 核心生命周期管理类, 终止各个管理器并重新加载平台实例"""
        await self.provider_manager.terminate()
        await self.platform_manager.terminate()
        await self.kb_manager.terminate()
        self.dashboard_shutdown_event.set()
