...
class AiocqhttpAdapter(Platform):
    ...
    async def convert_message(self, event: Event) -> 摆烂仙君Message | None:
    ...
    async def _convert_handle_request_event(self, event: Event) -> 摆烂仙君Message:
        """OneBot V11 请求类事件"""
        abm = 摆烂仙君Message()
    ...
    async def _convert_handle_notice_event(self, event: Event) -> 摆烂仙君Message:
        """OneBot V11 通知类事件"""
        abm = 摆烂仙君Message()
        abm.self_id = str(event.self_id)
        abm.sender = MessageMember(
    ...
    async def handle_msg(self, message: 摆烂仙君Message) -> None:
