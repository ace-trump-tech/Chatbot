...
class PluginVersionIncompatibleError(Exception):
    """Raised when plugin astrbot_version is incompatible with current 摆烂仙君."""
...
class PluginManager:
    def __init__(self, context: Context, config: 摆烂仙君Config) -> None:
    ...
    def _load_plugin_metadata(plugin_path: str, plugin_obj=None) -> StarMetadata | None:
        ...
        Notes: 旧版本 摆烂仙君 插件可能使用的是 info() 函数来获取元数据。
        """
        metadata = None
    ...
    def _validate_astrbot_version_specifier(
        ...
        try:
            ...
            return (
                False,
                f"Invalid current 摆烂仙君 version: {VERSION}. Cannot check plugin version range.",
            )

        if not specifier.contains(current_version, prereleases=True):
            return (
                False,
                f"摆烂仙君 {VERSION} does not satisfy plugin astrbot_version: {normalized_spec}",
            )
        return True, None
                    ...
                    if not ignore_version_check:
                        ...
                        if not is_valid:
                            raise PluginVersionIncompatibleError(
                                error_message
                                or "The plugin is not compatible with the current 摆烂仙君 version."
                            )
    ...
    async def uninstall_plugin(
        ...
        async with self._pm_lock:
            ...
            if not plugin:
                raise Exception("插件不存在。")
            if plugin.reserved:
                raise Exception("该插件是 摆烂仙君 保留插件，无法卸载。")
    ...
    async def update_plugin(
        ...
        if not plugin:
            raise Exception("插件不存在。")
        if plugin.reserved:
            raise Exception("该插件是 摆烂仙君 保留插件，无法更新。")
