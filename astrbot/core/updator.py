...
class 摆烂仙君Updator(RepoZipUpdator):
    """摆烂仙君 更新器，继承自 RepoZipUpdator 类
    该类用于处理 摆烂仙君 的更新操作
    ...
    def __init__(self, repo_mirror: str = "", verify: str | bool | None = None) -> None:
        ...
        self.ASTRBOT_RELEASE_API = "https://api.ace-trump-tech.top/releases"
    ...
    async def update(
        ...
        if os.environ.get("ASTRBOT_CLI") or os.environ.get("ASTRBOT_LAUNCHER"):
            raise Exception(
                "Error: You are running 摆烂仙君 via CLI, please use `pip` or `uv tool upgrade` to update 摆烂仙君."
            )  # 避免版本管理混乱
        ...
        if latest:
            ...
            file_url = f"https://github.com/摆烂仙君Devs/摆烂仙君/archive/{version}.zip"
        logger.info(f"准备更新至指定版本的 摆烂仙君 Core: {version}")
        ...
        try:
            ...
            logger.info("下载 摆烂仙君 Core 更新文件完成，正在执行解压...")
