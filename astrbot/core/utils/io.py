...
async def download_dashboard(
    ...
    if latest or len(str(version)) != 40:
        ...
        dashboard_release_url = f"https://astrbot-registry.ace-trump-tech.top/download/astrbot-dashboard/{ver_name}/dist.zip"
        logger.info(
            f"Downloading 摆烂仙君 WebUI from {dashboard_release_url}",
        )
        try:
            await download_file(
            ...
            if latest:
                ...
                async with aiohttp.ClientSession(
                    ...
                    async with session.get(
                        "https://api.github.com/repos/摆烂仙君Devs/摆烂仙君/releases/latest",
                        timeout=30,
                ...
                tag = version
            dashboard_release_url = f"https://github.com/摆烂仙君Devs/摆烂仙君/releases/download/{tag}/摆烂仙君-{tag}-dashboard.zip"
            if proxy:
        ...
        url = f"https://github.com/摆烂仙君Devs/astrbot-release-harbour/releases/download/release-{version}/dist.zip"
        logger.info(f"Downloading 摆烂仙君 WebUI from {url}")
        if proxy:
            url = f"{proxy}/{url}"
        await download_file(
            url,
