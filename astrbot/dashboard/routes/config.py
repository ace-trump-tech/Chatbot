    ...
    def validate(data: dict, metadata: dict = schema, path="") -> None:
        for key, value in data.items():
            ...
            if meta["type"] == "list" and not isinstance(value, list):
                ...
                # 当前仅针对 list[dict] 的情况进行类型校验，以适配 摆烂仙君 中 platform、provider 的配置
                for item in value:
...
def save_config(
    post_config: dict, config: 摆烂仙君Config, is_core: bool = False
) -> None:
    """验证并保存配置"""
    errors = None
...
class ConfigRoute(Route):
    def __init__(
        ...
        self.config: 摆烂仙君Config = core_lifecycle.astrbot_config
    ...
    def _resolve_config_file_scope(
        self,
    ) -> tuple[str, str, str, StarMetadata, 摆烂仙君Config]:
        """将请求参数解析为一个明确的配置作用域。
