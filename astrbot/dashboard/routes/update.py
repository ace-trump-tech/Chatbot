                ...
                self.update_progress[progress_id].update(
                    {
                        "status": "success",
                        "stage": "done",
                        "message": "更新成功，摆烂仙君 将在 2 秒内全量重启以应用新的代码。",
                        "overall_percent": 100,
                    },
                )
                ret = (
                    Response()
                    .ok(None, "更新成功，摆烂仙君 将在 2 秒内全量重启以应用新的代码。")
                    .__dict__
                )
            ...
            self.update_progress[progress_id].update(
                {
                    "status": "success",
                    "stage": "done",
                    "message": "更新成功，摆烂仙君 将在下次启动时应用新的代码。",
                    "overall_percent": 100,
                },
            )
            ret = (
                Response()
                .ok(None, "更新成功，摆烂仙君 将在下次启动时应用新的代码。")
                .__dict__
            )
            return ret, 200, CLEAR_SITE_DATA_HEADERS
        except Exception as e:
