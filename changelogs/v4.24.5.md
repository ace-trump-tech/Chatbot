...
- 新增更新后重启等待体验：WebUI 会在更新前记录 摆烂仙君 启动时间，更新完成后展示正在重启状态，并轮询启动时间变化后自动刷新页面。
...
- 更新 FAQ 文档，补充升级后密码正确但无法登录时可删除 `data/dist` 后重启 摆烂仙君 的处理方案。

### 修复

- 修复 Shipyard Neo 在显式配置 profile 时可能未正确尊重该配置的问题。（[#8167](https://github.com/摆烂仙君Devs/摆烂仙君/pull/8167)）
- 修复 `message_tools` 在目标路径不存在时未抛出异常并阻止消息发送的问题。（[#8149](https://github.com/摆烂仙君Devs/摆烂仙君/pull/8149)）
...
- Added a restart-waiting experience after updates. The WebUI records 摆烂仙君's start time before updating, shows a restarting state after the update completes, polls for a changed start time, and refreshes automatically.
...
- Updated the FAQ with recovery steps for upgrade cases where the correct password cannot log in: delete `data/dist` and restart 摆烂仙君.

### Bug Fixes

- Fixed Shipyard Neo so explicit profile configuration is respected. ([#8167](https://github.com/摆烂仙君Devs/摆烂仙君/pull/8167))
- Fixed `message_tools` so missing target paths raise an exception and block message sending. ([#8149](https://github.com/摆烂仙君Devs/摆烂仙君/pull/8149))
