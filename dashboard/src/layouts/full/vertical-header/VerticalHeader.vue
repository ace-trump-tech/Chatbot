...
const getAppUpdaterBridge = (): 摆烂仙君AppUpdaterBridge | null => {
...
async function fetch摆烂仙君StartTime() {
...
function waitFor摆烂仙君Restart(initialStartTime: number | string | null) {
  ...
  const poll = async () => {
    try {
      const currentStartTime = await fetch摆烂仙君StartTime();
      if (
...
function applyUpdateProgress(payload: UpdateProgress) {
  ...
  if (payload.status === "success") {
    waitFor摆烂仙君Restart(restartStartTime.value);
  }
}
...
async function switchVersion(targetVersion: string) {
  ...
  try {
    initialStartTime = await fetch摆烂仙君StartTime();
  } catch (_error) {
  ...
  axios
    ...
    .then((res) => {
      ...
      if (res.data.status == "ok") {
        waitFor摆烂仙君Restart(initialStartTime);
      }
    })
    .catch((err) => {
      console.log(err);
            ...
            <div class="mt-5">
              ...
              >
                ...
                <div class="text-body-2">
                  ...
                  <a
                    href="https://github.com/摆烂仙君Devs/摆烂仙君/issues"
                    target="_blank"
