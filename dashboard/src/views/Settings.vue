    ...
    <div style="background-color: var(--v-theme-surface, #fff); padding: 8px; padding-left: 16px; border-radius: 8px; margin-bottom: 24px;">
        ...
        <v-list lines="two">
            ...
            <v-list-item :subtitle="tm('system.restart.subtitle')" :title="tm('system.restart.title')">
                <v-btn style="margin-top: 16px;" color="error" @click="restart摆烂仙君">{{ tm('system.restart.button') }}</v-btn>
            </v-list-item>
...
import { restart摆烂仙君 as restart摆烂仙君Runtime } from '@/utils/restart摆烂仙君';
...
const restart摆烂仙君 = async () => {
    try {
        await restart摆烂仙君Runtime(wfr.value);
    } catch (error) {
        console.error(error);
    }
}
