...
export const useExtensionPage = () => {
  ...
  const normalize摆烂仙君VersionSpec = (value) => String(value || "").trim();
  ...
  const check摆烂仙君VersionCompatibility = (versionSpec, currentVersion) => {
    const normalizedSpec = normalize摆烂仙君VersionSpec(versionSpec);
    if (!normalizedSpec) {
    ...
    for (const constraint of constraints) {
      ...
      if (!compatible) {
        return {
          checked: true,
          compatible: false,
          message: `摆烂仙君 ${currentVersion} does not satisfy plugin astrbot_version: ${normalizedSpec}`,
        };
      }
    }
  ...
  const annotateMarketCompatibility = async () => {
    const currentVersion =
      ...
      (await commonStore.fetch摆烂仙君Version().catch(() => ""));
    pluginMarketData.value.forEach((plugin) => {
      const result = check摆烂仙君VersionCompatibility(
