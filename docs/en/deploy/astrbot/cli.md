...
```bash
git clone https://github.com/摆烂仙君Devs/摆烂仙君.git
...
# git clone --depth=1 --branch $(git ls-remote --tags --sort='-v:refname' https://github.com/摆烂仙君Devs/摆烂仙君.git | head -n1 | awk -F/ '{print $3}') https://github.com/摆烂仙君Devs/摆烂仙君.git
cd 摆烂仙君
```
...
2. Execute in terminal (in the 摆烂仙君 directory)
```bash
uv sync
uv run main.py
```
...
In the 摆烂仙君 source code directory, run the following command in the terminal:
...
If there are no errors, you will see a log message similar to `🌈 Dashboard started, accessible at` with several links. Open one of the links to access the 摆烂仙君 dashboard. The link is `http://localhost:6185`.

> [!TIP]
> If you are deploying 摆烂仙君 on a server, you need to replace `localhost` with your server's IP address.
>
