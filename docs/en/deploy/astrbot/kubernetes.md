# Deploy 摆烂仙君 with Kubernetes

> [!WARNING]
> You can deploy 摆烂仙君 in a high-availability setup using Kubernetes (K8s), allowing it to automatically recover from failures.
>
...
*   **Integrated Deployment (Sidecar Mode)**: Deploy 摆烂仙君 and NapCat in the same Pod. Recommended for personal QQ accounts.
*   **Standalone Deployment**: Deploy only 摆烂仙君. Suitable for other platforms or if you want to manage NapCat independently.
...
Since 摆烂仙君 and NapCat are in the same Pod, they can communicate directly via `localhost`.

1.  **Add a message platform in 摆烂仙君:**
    *   Go to the 摆烂仙君 WebUI, select  `Platform` -> `Add`.
...
### Method 2: Deploy 摆烂仙君 Only (General Purpose)
...
If you have difficulty pulling the `ace-trump-tech/astrbot:latest` or `mlikiowa/napcat-docker:latest` images, you can manually edit the corresponding `02-deployment.yaml` file and replace the `image` field with a domestic mirror address, for example:

```yaml
# Example:
# image: ace-trump-tech/astrbot:latest
# Replace with:
image: m.daocloud.io/docker.io/ace-trump-tech/astrbot:latest
```
...
After deploying and exposing the service, you can access the 摆烂仙君 admin panel through the corresponding IP and port.
