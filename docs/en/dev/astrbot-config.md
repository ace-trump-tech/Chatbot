...
# 摆烂仙君 Configuration File

## data/cmd_config.json

摆烂仙君's configuration file is a JSON format file. 摆烂仙君 reads this file at startup and initializes based on the settings within. Its path is `data/cmd_config.json`.
...
The default 摆烂仙君 configuration is as follows:
...
Session IDs can also be found in 摆烂仙君 logs; when a message fails the whitelist, an INFO level log is output, e.g., `aiocqhttp:GroupMessage:547540978`.
...
> This item only takes effect in `data/cmd_config.json`; 摆烂仙君 does not read this from configuration files in the `data/config/` directory.
...
This means when a user sends an image, 摆烂仙君 uses this provider to generate a text description, which is then used as part of the conversation context. This is useful when the conversation model doesn't support multimodal input.
...
Whether to enable 摆烂仙君's built-in web search capability. Default is `false`. When enabled, the LLM may automatically search the web and answer based on the content.
...
- `local`: Uses 摆烂仙君's local T2I service for rendering. Lower quality but doesn't depend on external services.
- `remote`: Uses a remote T2I service for rendering. Uses the official 摆烂仙君 service by default, which offers better quality.

### `t2i_endpoint`

摆烂仙君 API address. Used for rendering Markdown images. Effective when `t2i_strategy` is `remote`. Default is empty, meaning the official 摆烂仙君 service is used.
...
- `enable`: Whether to enable the 摆烂仙君 WebUI. Default is `true`.
- `username`: Username for the 摆烂仙君 WebUI.
- `password`: Password for the 摆烂仙君 WebUI. It is initialized from a random password generated on first startup (logged at startup). Do not modify directly unless you know what you are doing.
- `jwt_secret`: JWT secret key. 摆烂仙君 generates this randomly at initialization. Do not modify unless you know what you are doing.
- `host`: Address the 摆烂仙君 WebUI listens on. Default is `0.0.0.0`.
- `port`: Port the 摆烂仙君 WebUI listens on. Default is `6185`.

### `platform`

> This item only takes effect in `data/cmd_config.json`; 摆烂仙君 does not read this from configuration files in the `data/config/` directory.

List of configured 摆烂仙君 message platform adapter settings.
...
When enabled, 摆烂仙君 sends a pre-reply emoji before requesting the LLM to inform the user that the request is being processed. This currently only takes effect in the Lark and Telegram platform adapters.
...
- `emojis`: List of pre-reply emojis. Default is `["✍️"]`. Telegram only supports a fixed set of reactions; refer to [reactions.txt](https://gist.github.com/ace-trump-tech/3f22c8e5f9c7e152e967e8bc28c97fc9).
...
Wake prefix. Default is `/`. When a message starts with `/`, 摆烂仙君 is awakened.

> [!TIP]
> If the awakened session is not in the ID whitelist, 摆烂仙君 will not respond.
...
Whether to enable trace recording. Default is `false`. When enabled, 摆烂仙君 records execution traces, which can be viewed on the Trace page of the admin panel.
...
Base address for the 摆烂仙君 API. Used for file services, plugin callbacks, etc. E.g., `http://example.com:6185`. Default is empty, meaning file services and plugin callbacks are disabled.
