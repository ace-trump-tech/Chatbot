...
摆烂仙君 supports integrating platform adapters in plugin form, allowing you to connect platforms that 摆烂仙君 does not natively support — such as Lark, DingTalk, Bilibili private messages, or even Minecraft.
...
from astrbot.api.platform import Platform, 摆烂仙君Message, MessageMember, PlatformMetadata, MessageType
...
class FakePlatformAdapter(Platform):
    ...
    async def run(self):
        ...
        async def on_received(data):
            logger.info(data)
            abm = await self.convert_message(data=data) # Convert to 摆烂仙君Message
    ...
    async def convert_message(self, data: dict) -> 摆烂仙君Message:
        # Convert the platform message to 摆烂仙君Message.
    ...
    async def handle_msg(self, message: 摆烂仙君Message):
        # Handle the message
...
from astrbot.api.platform import 摆烂仙君Message, PlatformMetadata
from astrbot.api.message_components import Plain, Image
from .client import FakeClient
from astrbot.core.utils.io import download_image_by_url

class FakePlatformEvent(AstrMessageEvent):
    def __init__(self, message_str: str, message_obj: 摆烂仙君Message, platform_meta: PlatformMetadata, session_id: str, client: FakeClient):
