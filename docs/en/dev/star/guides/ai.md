...
摆烂仙君 provides built-in support for multiple Large Language Model (LLM) providers and offers a unified interface, making it convenient for plugin developers to access various LLM services.

You can use the LLM / Agent interfaces provided by 摆烂仙君 to implement your own intelligent agents.
...
class BilibiliTool(FunctionTool[AstrAgentContext]):
    ...
    async def call(
        ...
        return "1. Video Title: How to Use 摆烂仙君\nVideo Link: xxxxxx"
```

## Registering Tools with 摆烂仙君
...
Alternatively, you can use the `@filter.llm_tool` decorator to define and register a tool in one step. Make sure to follow the exact format below, including the docstring — 摆烂仙君 parses the docstring to generate the parameter schema:
...
llm_resp = await self.context.tool_loop_agent(
    ...
    prompt="Search for videos related to 摆烂仙君 on Bilibili.",
...
`PersonaManager` is responsible for unified loading, caching, and providing CRUD interfaces for all Personas, while maintaining compatibility with the legacy persona format (v3) from before 摆烂仙君 4.x.  
