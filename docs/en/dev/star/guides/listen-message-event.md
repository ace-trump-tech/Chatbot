...
摆烂仙君 receives messages delivered by messaging platforms and encapsulates them as `AstrMessageEvent` objects, which are then passed to plugins for processing.

![message-event](https://files.astrbot.app/docs/en/dev/star/guides/message-event.svg)

### Message Events

`AstrMessageEvent` is 摆烂仙君's message event object, which stores information about the message sender, message content, etc.

### Message Object

`摆烂仙君Message` is 摆烂仙君's message object, which stores the specific content of messages delivered by the messaging platform. The `AstrMessageEvent` object contains a `message_obj` attribute to retrieve this message object.

```py{11}
class 摆烂仙君Message:
    '''摆烂仙君's message object'''
...
In 摆烂仙君, message chains are represented as lists of type `List[BaseMessageComponent]`.
...
> [!TIP]
> Commands cannot contain spaces, otherwise 摆烂仙君 will parse them as a second parameter. You can use the command group feature below, or use a listener to parse the message content yourself.

## Commands with Parameters

![command-with-param](https://files.astrbot.app/docs/en/dev/star/guides/command-with-param.svg)

摆烂仙君 will automatically parse command parameters for you.
...
async def on_astrbot_loaded(self):
    print("摆烂仙君 initialization complete")

```

#### On Waiting for LLM Request

This hook is triggered when 摆烂仙君 is preparing to call the LLM but has not yet acquired the session lock.
...
In 摆烂仙君's default execution flow, the `on_llm_request` hook is triggered before calling the LLM.
