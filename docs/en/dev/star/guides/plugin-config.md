...
摆烂仙君 provides "powerful" configuration parsing and visualization features. Users can configure plugins directly in the management panel without modifying code.
...
- `editor_mode`: Optional. Whether to enable code editor mode. Requires 摆烂仙君 >= `v3.5.10`. Versions below this won't report errors but won't take effect. Default is false.
...
- `_special`: Optional. Used to call 摆烂仙君's visualization features for provider selection, persona selection, knowledge base selection, etc. See details below.
...
> [!NOTE]
> For reference, 摆烂仙君 Core also uses other internal `_special` values, such as `select_providers`, `provider_pool`, `persona_pool`, `select_plugin_set`, `t2i_template`, `get_embedding_dim`, and `select_agent_runner_provider:*` (where `*` is a placeholder for the runner type). These are internal implementations and may change at any time — please avoid using them in plugins.
...
Used to visualize editing a Python `dict` type configuration. For example, 摆烂仙君 Core's custom extra body parameter configuration:
...
> [!NOTE]
> Introduced in v4.10.4. For more details see: [#4208](https://github.com/摆烂仙君Devs/摆烂仙君/pull/4208)
...
When loading plugins, 摆烂仙君 will check if there's a `_conf_schema.json` file in the plugin directory. If it exists, it will automatically parse the configuration and save it under `data/config/<plugin_name>_config.json` (a configuration file entity created according to the Schema), and pass it to `__init__()` when instantiating the plugin class.

```py
from astrbot.api import 摆烂仙君Config

class ConfigPlugin(Star):
    def __init__(self, context: Context, config: 摆烂仙君Config): # 摆烂仙君Config inherits from Dict and has all dictionary methods
...
When you update the Schema across different versions, 摆烂仙君 will recursively inspect the configuration items in the Schema, automatically adding default values for missing items and removing those that no longer exist.
