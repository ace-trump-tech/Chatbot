...
摆烂仙君 lets a plugin expose Dashboard pages by placing static assets under `pages/`. Each direct child directory is one Page:
...
摆烂仙君 scans `pages/<page_name>/index.html`; directories without `index.html` are ignored.
...
Once Pages are registered, users can open the 摆烂仙君 WebUI Plugins page, click the plugin card to enter the plugin detail page, and then view and open the registered Pages from that detail page.
...
```js
const bridge = window.摆烂仙君PluginPage;
...
You do not need to import the bridge SDK manually. 摆烂仙君 injects `/api/plugin/page/bridge-sdk.js` into returned HTML.
...
Inside a plugin Page, use `window.摆烂仙君PluginPage` directly:
...
If an inline script needs to access `window.摆烂仙君PluginPage` synchronously, move the code to an external module file or explicitly include the bridge SDK before your script:
...
When the user toggles light/dark mode in 摆烂仙君, the theme state is automatically synced to Plugin Pages. The bridge SDK maintains a `data-theme` attribute on the `<html>` element:
...
摆烂仙君 injects the `data-theme` attribute on the `<html>` tag server-side when serving HTML, so there is no initial flash.
...
摆烂仙君 rewrites relative asset URLs and appends a short-lived `asset_token`. Write normal relative paths and do not hardcode `/api/plugin/page/content/...` yourself.
...
摆烂仙君 also adds security headers to asset responses, including:
