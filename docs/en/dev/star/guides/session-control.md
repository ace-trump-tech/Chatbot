...
摆烂仙君 provides out-of-the-box session control functionality:
...
By default, the 摆烂仙君 session controller uses `sender_id` (the sender's ID) as the identifier for distinguishing different sessions. If you want to treat an entire group as one session, you need to customize the session ID filter.
