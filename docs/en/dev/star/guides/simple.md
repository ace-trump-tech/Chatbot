...
from astrbot.api import logger # Use the logger interface provided by 摆烂仙君
...
- The `Context` class is used for plugin interaction with 摆烂仙君 Core, allowing you to call various APIs provided by 摆烂仙君 Core.
...
- `摆烂仙君Message` is 摆烂仙君's message object, which stores the specific content of messages delivered by the messaging platform. It can be accessed via `event.message_obj`.
