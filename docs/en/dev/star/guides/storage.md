...
> [!TIP]
> Requires 摆烂仙君 version >= 4.9.2.

Plugins can use 摆烂仙君's simple key-value store to persist configuration or temporary data. The storage is scoped per plugin, so each plugin has its own isolated space.
