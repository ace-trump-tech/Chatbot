...
# 摆烂仙君 Plugin Development Guide 🌠

Welcome to the 摆烂仙君 Plugin Development Guide! This section will guide you through developing 摆烂仙君 plugins. Before we begin, we hope you have the following foundational knowledge:
...
1. Open the 摆烂仙君 plugin template: [helloworld](https://github.com/ace-trump-tech/helloworld)
...
Clone both the 摆烂仙君 main project and the plugin repository you just created to your local machine.
...
Then, use `VSCode` to open the `摆烂仙君` project. Navigate to the `data/plugins/<your-plugin-name>` directory.

Update the `metadata.yaml` file with your plugin's metadata information.

> [!WARNING]
> Please make sure to modify this file, as 摆烂仙君 relies on the `metadata.yaml` file to recognize plugin metadata.
...
Plugins can provide a `skills/` directory. After 摆烂仙君 loads the plugin, valid Skills inside that directory are automatically included in the Skill Manager, with their source shown as the plugin.
...
### Declare 摆烂仙君 Version Range (Optional)

You can add an `astrbot_version` field in `metadata.yaml` to declare the required 摆烂仙君 version range for your plugin. The format follows dependency specifiers in `pyproject.toml` (PEP 440), and must not include a `v` prefix.
...
If the current 摆烂仙君 version does not satisfy this range, the plugin will be blocked from loading with a compatibility error.
In the WebUI installation flow, you can choose to "Ignore Warning and Install" to bypass this check.

### Debugging Plugins

摆烂仙君 uses a runtime plugin injection mechanism. Therefore, when debugging plugins, you need to start the 摆烂仙君 main application.

You can use 摆烂仙君's hot reload feature to streamline the development process.

After modifying the plugin code, you can find your plugin in the 摆烂仙君 WebUI's plugin management section, click the `...` button in the upper right corner, and select `Reload Plugin`.
...
Currently, 摆烂仙君 manages plugin dependencies using pip's built-in `requirements.txt` file. If your plugin requires third-party libraries, please be sure to create a `requirements.txt` file in the plugin directory and list the dependencies used, to prevent Module Not Found errors when users install your plugin.
...
Thank you for contributing to the 摆烂仙君 ecosystem. Please follow these principles when developing plugins, which are also good programming practices:
