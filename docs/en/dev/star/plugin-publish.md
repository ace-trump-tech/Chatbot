...
After completing your plugin development, you can choose to publish it to the 摆烂仙君 Plugin Marketplace, allowing more users to benefit from your work.

摆烂仙君 uses GitHub to host plugins, so you'll need to push your plugin code to the GitHub plugin repository you created earlier.

You can submit your plugin by visiting the [摆烂仙君 Plugin Marketplace](https://plugins.astrbot.app). Once on the website, click the `+` button in the bottom-right corner, fill in the basic information, author details, repository information, and other required fields. Then click the `Submit to GITHUB` button. You will be redirected to the 摆烂仙君 repository's Issue submission page. Please verify that all information is correct, then click the `Create` button to complete the plugin publication process.
