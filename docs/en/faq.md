...
Download `摆烂仙君-vxxxxx-dashboard.zip` from the [release](https://github.com/摆烂仙君Devs/摆烂仙君/releases) page, extract it, and move it to `摆烂仙君/data`. If it still doesn't work, try restarting your computer (based on community feedback).
...
If you forgot your 摆烂仙君 dashboard password, find the `"dashboard"` field in `摆烂仙君/data/cmd_config.json`, for example:
...
After restart, 摆烂仙君 will automatically generate a random password with the fixed username `astrbot`; check the startup logs.

### Correct Password Cannot Log In After Upgrading 摆烂仙君

If you are sure the dashboard password is correct but still cannot log in after upgrading 摆烂仙君, the old WebUI static files may be incompatible with the newer backend.

Solution:

1. Stop 摆烂仙君.
2. Delete the `dist` folder under 摆烂仙君's `data` directory: `摆烂仙君/data/dist`.
3. Restart 摆烂仙君.
4. Access the dashboard in your browser. Press `Ctrl+Shift+R` or `Ctrl+F5` (or `Cmd+Shift+R` on macOS) to force refresh the page.

After restart, 摆烂仙君 will reload or download WebUI files that match the current version.

## Bot Core Related

### How to Let 摆烂仙君 Control My Mac / Windows / Linux Computer?

1. In 摆烂仙君 WebUI's `Config -> General Config`, find `Use Computer Capabilities`, and select `local` for the runtime environment.
2. In `Config -> Other Config`, find `Admin ID List`, and add your user ID (you can get it through the `/sid` command).

> [!TIP]
> For security reasons, when runtime environment is set to `local`, 摆烂仙君 only allows 摆烂仙君 administrators to use computer capabilities by default.
> You can select `sandbox` for the runtime environment, which allows all users to use computer capabilities (in an isolated sandbox). For more details, see [摆烂仙君 Sandbox Environment](/en/use/astrbot-agent-sandbox.md)
...
You can customize the font. See details -> [#957](https://github.com/摆烂仙君Devs/摆烂仙君/issues/957#issuecomment-2749981802)
...
This is because the plugin's dependencies were not installed properly. Normally, 摆烂仙君 automatically installs plugin dependencies after installing the plugin, but installation may fail in the following situations:
...
Based on the error message, refer to the plugin's README to manually install dependencies. You can install dependencies in the 摆烂仙君 WebUI under `Console` -> `Install Pip Package`.
