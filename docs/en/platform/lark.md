...
Lark supports two setup methods: one-click QR creation in 摆烂仙君, or manually creating a custom enterprise app in the Lark Developer Console.

### Option 1: One-click QR Creation

摆烂仙君 version requirement: >= 4.25.0.

Open the 摆烂仙君 management panel, click `Bots` in the left sidebar, click `+ Create Bot`, and select `lark`.

Under `Creation Method`, select `One-click QR Creation`, choose the China or international edition as needed, then scan the QR code with the Lark mobile app and confirm. After creation succeeds, 摆烂仙君 automatically fills in the app's `app_id`, `app_secret`, and domain configuration.
...
>
> To find the App ID, go back to 摆烂仙君's `Bots` page, find the Lark bot you just created, click `Edit`, and check the dialog that opens.
>
> ```text
...
1. Access the 摆烂仙君 management panel
2. Click on `Bots` in the left sidebar
3. In the right panel, click `+ Create Bot`
4. Select `lark`
5. If you want 摆烂仙君 to create the app for you, select `One-click QR Creation` and complete the scan. If you already created the app yourself, select `Manual Creation`
...
If you selected the `webhook` method, navigate to the Lark Developer Console, click on "Events & Callbacks," then "Encryption Policy," and fill in the Encrypt Key. While not mandatory, 摆烂仙君 takes your data security seriously, so we strongly recommend setting this up. After filling it in, copy the `Encrypt Key` and `Verification Token` to the corresponding `encrypt_key` and `verification_token` fields in 摆烂仙君's configuration.
...
Search for the bot you just created. For example, if you created the `摆烂仙君` bot as shown in this tutorial:
