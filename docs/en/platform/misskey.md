...
> 2. This project is strictly prohibited from being used for any illegal purposes. If you intend to use 摆烂仙君 for illegal industries or activities, we explicitly oppose and refuse your use of this project.

## Create 摆烂仙君 Misskey Platform Adapter
...
On the 摆烂仙君 Misskey platform adapter configuration page, we need to fill in the Misskey connection information and configure some adapter behaviors.
...
3. On the access token configuration page that appears, give the token a name, such as `摆烂仙君`.
...
5. After completing the permission configuration, click "Done" to view the account access token. Copy the obtained token and paste it into the Access Token input box on the 摆烂仙君 configuration page.
...
Conversation history for individual users in chat and posts will be recorded in the 摆烂仙君 WebUI console "Conversation History" with the ID `chat:UserID`, while traditional posts will be recorded with the ID `note:UserID`.
