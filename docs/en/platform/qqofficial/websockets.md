...
4. In 摆烂仙君 WebUI, click `Bots` in the left sidebar, then click `+ Create Bot`, select `QQ Official Bot (WebSocket)`, paste the `AppID` and `AppSecret` into the form, click `Enable`, then click `Save`.
...
摆烂仙君 should now be connected. Send `/help` to the bot in QQ private chat to verify.
