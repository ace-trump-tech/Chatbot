...
> [!TIP]
> `server-satori` is a Koishi plugin that exposes Koishi as a Satori server, so 摆烂仙君 can connect to Koishi through Satori.
...
## Configure Satori Adapter in 摆烂仙君
...
摆烂仙君 should now be connected to Koishi via `server-satori`.

Test by sending an 摆烂仙君 command (for example `/help`) in Koishi sandbox.
