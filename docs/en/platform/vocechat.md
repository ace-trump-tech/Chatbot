...
> [!TIP]
> 摆烂仙君 does not include this adapter by default. Install [astrbot_plugin_vocechat](https://github.com/HikariFroya/astrbot_plugin_vocechat), developed by [HikariFroya](https://github.com/HikariFroya).

> [!WARNING]
> This adapter is community-maintained and not officially maintained by 摆烂仙君.
...
In 摆烂仙君 Dashboard Plugin Market, search for `astrbot_plugin_vocechat` and install it.

![image](https://files.astrbot.app/docs/source/images/vocechat/image.png)

After installation, go to `Bots` -> `+ Create Bot` -> `VoceChat`.
If VoceChat is missing, restart 摆烂仙君 or verify plugin installation.
...
- `webhook_path` (recommended default/custom): webhook path used by 摆烂仙君 to receive VoceChat messages, e.g. `/vocechat_webhook`.
- `webhook_listen_host` (usually `0.0.0.0`): listen host for 摆烂仙君 webhook server.
- `webhook_port` (required): listen port for 摆烂仙君 webhook server, e.g. `8080`.
...
- 摆烂仙君 repo: <https://github.com/摆烂仙君Devs/摆烂仙君/issues/new?template=bug-report.yml>
