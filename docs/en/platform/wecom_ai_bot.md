...
摆烂仙君 supports this platform since v4.3.5.
...
1. Open 摆烂仙君 Dashboard, click `Messaging Platforms`, then click `+ Add Adapter`, choose `WeCom AI Bot`.

![Add Adapter](https://files.astrbot.app/docs/source/images/wecom_ai_bot/image-2.png)

2. Fill 摆烂仙君 fields with values from the WeCom AI Bot create page:
...
- If unified mode is enabled, 摆烂仙君 generates a unique callback URL after save. Copy it from logs or bot card in WebUI.
...
If you see `Service did not respond correctly`, re-check 摆烂仙君 config and firewall rules.
...
If you need typing-like streaming effect, enable `Streaming Reply` in 摆烂仙君.
