# Connect 摆烂仙君 to WeChat Official Account Platform

摆烂仙君 supports WeChat Official Account integration (version >= v3.5.8). After setup, you can chat with 摆烂仙君 directly in the WeChat Official Account chat interface.
...
Copy AppID and AppSecret from WeChat platform to 摆烂仙君 fields `appid` and `secret`.

Open IP whitelist and add your public IP(s), one per line if multiple.

In server configuration, click modify.

- `Token`: create any string with length 3-32, and fill the same value in 摆烂仙君 `token`.
- `EncodingAESKey`: click random generate and fill 摆烂仙君 `encoding_aes_key`.

Keep `Unified Webhook Mode (unified_webhook_mode)` enabled (recommended), then save 摆烂仙君 config and wait for restart.

For `URL`:

- If unified mode is enabled, use the unique callback URL generated by 摆烂仙君 (from logs or bot card).
...
> - Unified mode enabled: forward to 摆烂仙君 port `6185`
...
Scan it with WeChat, send `help`, and check whether 摆烂仙君 replies.
...
摆烂仙君 supports custom endpoint via `api_base_url` for environments without stable public IP.
