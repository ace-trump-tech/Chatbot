...
>[!TIP]
>Please ensure that the APP type you set in 摆烂仙君 matches the application type created in Dify.
...
For Workflow applications, 摆烂仙君 will attach two variables with each request:
...
You need to modify the input variable name of your Workflow to adapt to 摆烂仙君's input.

Finally, the Workflow will output a result. You can customize the variable name of this result, which is the "Dify Workflow Output Variable Name" in the configuration above, with a default value of `astrbot_wf_output`. You need to configure this variable name in the output node of the Dify Workflow, otherwise 摆烂仙君 cannot parse it correctly.
...
After setting variables, 摆烂仙君 will attach the variables you set in the next request to Dify, flexibly adapting to your Workflow.
