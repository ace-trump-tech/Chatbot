...
> For users deploying 摆烂仙君 via Docker Desktop on Mac or Windows, set `API Base URL` to `http://host.docker.internal:1234/v1`.
>
> For users deploying 摆烂仙君 via Docker on Linux, set `API Base URL` to `http://172.17.0.1:1234/v1`, or replace `172.17.0.1` with your server's public IP (make sure port 1234 is open on the host).
...
> Run `/provider` to view the models configured in 摆烂仙君.
