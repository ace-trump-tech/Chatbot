...
Open the 摆烂仙君 WebUI, locate Service Provider Management, click on Add Provider, find and click on `Ollama`.
...
For Mac/Windows users deploying 摆烂仙君 with Docker Desktop, enter `http://host.docker.internal:11434/v1` for the API Base URL.\
For Linux users deploying 摆烂仙君 with Docker, enter `http://172.17.0.1:11434/v1` for the API Base URL, or replace `172.17.0.1` with your public IP address (ensure that port 11434 is allowed by the host system).\
