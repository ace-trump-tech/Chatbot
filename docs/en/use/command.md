...
摆烂仙君 commands are registered through the plugin system. To keep the core lightweight, only a small set of basic commands are loaded with 摆烂仙君 itself. Other management and extended commands have been moved into a separate plugin.
...
The following commands are shipped with 摆烂仙君 and loaded by default:

- `/help`: View currently enabled commands and 摆烂仙君 version information.
...
- `/dashboard_update`: Update 摆烂仙君 WebUI. This command requires admin permission.
...
- `UID`: The sender's user ID. It is commonly used when adding 摆烂仙君 admins.
...
- If both alias and auto name exist, 摆烂仙君 displays `alias (auto name)`.
- If only the auto name exists, 摆烂仙君 displays the auto name.
- If neither exists, 摆烂仙君 displays the raw UMO.
...
For 摆烂仙君's built-in Agent Runner, it:
...
If there are no running tasks in the current session, 摆烂仙君 will report that no task is running.
...
- [builtin_commands_extension](https://github.com/摆烂仙君Devs/builtin_commands_extension)
...
Some commands require 摆烂仙君 admin permission, such as `/dashboard_update`, `/name`, `/op`, `/deop`, `/provider`, `/model`, and `/persona`.
