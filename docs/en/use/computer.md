...
- `local`: executes on the host machine where 摆烂仙君 is running. Use this when the Agent needs local files, command-line tools, or local dependencies.
...
`local` mode mounts Computer Use tools into the host environment where 摆烂仙君 runs. The Agent can call the host Shell, host Python, and host filesystem tools.

This means the Agent's boundary is close to the 摆烂仙君 process itself. What it can access depends on the system permissions, runtime user, working directory, and operating-system restrictions of the 摆烂仙君 process.

### Workspace

In `local` mode, 摆烂仙君 prepares a workspace for each session:
...
> [!NOTE]
> The local Python tool executes code through 摆烂仙君's current Python environment. When Python code reads or writes files, use explicit absolute paths or prepare files through filesystem tools in the workspace first.
...
- `Python`: executes Python code in 摆烂仙君's current Python environment.
...
- 摆烂仙君 temporary directories
- `.astrbot` under the system temporary directory

If `Require 摆烂仙君 admin permission` is disabled, regular users behave much closer to admins for Computer Use tools. Do not disable it unless you understand the risk.
...
`sandbox` mode runs execution actions inside an isolated environment instead of directly on the 摆烂仙君 host.

Inside the sandbox, the Agent can still use Shell, Python, and filesystem tools. If the selected sandbox profile supports the `browser` capability, 摆烂仙君 also mounts browser automation tools.
...
> [!NOTE]
> Even in `sandbox` mode, `Require 摆烂仙君 admin permission` still affects access to Shell, Python, browser, upload/download, and related tools. The exact behavior depends on your configuration.
...
In `sandbox` mode, 摆烂仙君 attempts to sync local Skills into the sandbox so the Agent can execute them there.
