...
摆烂仙君 introduces a Proactive Agent system, enabling 摆烂仙君 to not only respond passively to users but also schedule future tasks and proactively execute them at specified times, delivering results (text, images, files, etc.) to users.
...
- **Self-Wakeup**: 摆烂仙君 automatically wakes up at the scheduled time to execute tasks.
- **Task Feedback**: After execution, 摆烂仙君 reports the results back to the task creator.
...
You can view and manage all future tasks by clicking **Future Tasks** in the left navigation bar of the 摆烂仙君 WebUI.

![](https://files.astrbot.app/docs/source/images/proactive-agent/image-1.png)

### Supported Platforms

Scheduling tasks is supported on all platforms. However, due to some platforms not providing APIs for proactive message pushing, only the following platforms support 摆烂仙君 proactively pushing results to users:
- Telegram
- OneBot (QQ)
- Slack
- Feishu (Lark)
...
To make it easier for Agents to send images, audio, video, and other files directly to users, 摆烂仙君 provides a `send_message_to_user` tool by default.
