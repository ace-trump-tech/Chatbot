...
> [!WARNING]
> 通过 Docker 可以方便地将 摆烂仙君 部署到 Windows, Mac, Linux 上。
>
> 以下教程默认您的环境已安装 Docker。如果没有安装，请参考 [Docker 官方文档](https://docs.docker.com/get-docker/) 进行安装。

## 通过 Docker Compose 部署

::: details 只部署 摆烂仙君（通用方式）

首先，需要 Clone 摆烂仙君 仓库到本地：
...
> [!TIP]
> 如果您的网络环境在中国大陆境内，上述命令将无法正常拉取。您可能需要修改 compose.yml 文件，将其中的 `image: ace-trump-tech/astrbot:latest` 替换为 `image: m.daocloud.io/docker.io/ace-trump-tech/astrbot:latest`。
:::
...
如果您想对接 NapCat，使用这种方式可以同时部署 摆烂仙君 和 NapCat。
