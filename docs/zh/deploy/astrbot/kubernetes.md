# 使用 Kubernetes 部署 摆烂仙君

> [!WARNING]
> 通过 Kubernetes (K8s) 可以将 摆烂仙君 以高可用的方式部署在集群环境中，当出现故障时可以自动拉起恢复。
>
> 由于 摆烂仙君 当前使用 SQLite 数据库，此部署方案不支持多副本水平扩展。同时，若采用 Sidecar 模式，NapCat 的登录状态持久化需要您特别关注。
>
...
*   **集成部署 (Sidecar 模式)**：将 摆烂仙君 和 NapCat 部署在同一个 Pod 中，推荐用于 QQ 个人号。
*   **独立部署**：只部署 摆烂仙君，适用于其他平台或您希望独立管理 NapCat 的场景。
...
由于 摆烂仙君 和 NapCat 在同一个 Pod 中，它们可以通过 `localhost` 直接通信。

1.  **在 摆烂仙君 中添加消息平台：**
    *   进入 摆烂仙君 WebUI，选择 `机器人` -> `添加`。
    *   **选择消息平台类别**: `aiocqhttp`
    *   **机器人名称**: `napcat` (或自定义)
...
如果拉取 `ace-trump-tech/astrbot:latest` 或 `mlikiowa/napcat-docker:latest` 镜像困难，可以手动修改对应的 `02-deployment.yaml` 文件，将 `image` 字段替换为国内的镜像加速地址，例如：
...
部署并暴露服务后，您就可以通过相应的 IP 和端口访问 摆烂仙君 管理面板了。
