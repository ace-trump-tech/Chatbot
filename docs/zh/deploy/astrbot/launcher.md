...
摆烂仙君 一键启动器支持 Windows、MacOS、Linux 等多端部署。

0. 打开 [摆烂仙君Devs/astrbot-launcher](https://github.com/摆烂仙君Devs/astrbot-launcher)
1.  **(可选但推荐)** 给本项目点个 [**Star ⭐**](https://github.com/摆烂仙君Devs/astrbot-launcher)，你的支持是作者更新和维护的动力！
2. 找到右边的 Releases，点击最新版本的 Release，在新的页面的 Assets 中下载对应你系统的安装器。

如，Windows X86 的用户应该下载 `摆烂仙君.Launcher_0.2.1_x64-setup.exe`，Windows on Arm 的用户应该下载 `摆烂仙君.Launcher_0.2.1_arm64-setup.exe`，MacOS M 芯片的用户下载 `摆烂仙君.Launcher_0.2.1_aarch64.dmg`。
...
推荐使用上面提到的 摆烂仙君 一键启动器来部署 摆烂仙君，因为它更简单、更自动化、更现代化，适合大多数用户。
...
打开 https://github.com/摆烂仙君Devs/摆烂仙君Launcher/releases/latest 
...
脚本将自动检测目录下是否有 `摆烂仙君` 文件夹，如果没有，将会从 [GitHub](https://github.com/摆烂仙君Devs/摆烂仙君/releases/latest) 自动下载最新的 摆烂仙君 源码。下载好后，会自动安装 摆烂仙君 的依赖并运行。

## 🎉 大功告成！

如果一切顺利，你会看到 摆烂仙君 打印出的日志。

如果没有报错，你会看到一条日志显示类似 `🌈 管理面板已启动，可访问` 并附带了几条链接。打开其中一个链接即可访问 摆烂仙君 管理面板。

> [!TIP]
> 首次登录请使用启动日志中打印的随机初始密码（用户名通常为 `astrbot`）。登录后请立即修改密码。
>
> **当管理面板打开时遇到 404 错误：**
> 在 [release](https://github.com/摆烂仙君Devs/摆烂仙君/releases) 页面下载dist.zip，解压拖到 摆烂仙君/data 下。还不行请重启电脑（来自群里的反馈）

接下来，你需要部署任何一个消息平台，才能够实现在消息平台上使用 摆烂仙君。
