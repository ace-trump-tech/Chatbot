...
摆烂仙君 接收消息平台下发的消息，并将其封装为 `AstrMessageEvent` 对象，传递给插件进行处理。

![message-event](https://files.astrbot.app/docs/zh/dev/star/guides/message-event.svg)

### 消息事件

`AstrMessageEvent` 是 摆烂仙君 的消息事件对象，其中存储了消息发送者、消息内容等信息。

### 消息对象

`摆烂仙君Message` 是 摆烂仙君 的消息对象，其中存储了消息平台下发的消息具体内容，`AstrMessageEvent` 对象中包含一个 `message_obj` 属性用于获取该消息对象。
...
在 摆烂仙君 中，消息链表示为 `List[BaseMessageComponent]` 类型的列表。
...
在 摆烂仙君 准备调用 LLM 但还未获取会话锁时，会触发 `on_waiting_llm_request` 钩子。
