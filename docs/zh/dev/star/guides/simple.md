...
- 插件需要继承 `Star` 类。
- `Context` 类用于插件与 摆烂仙君 Core 交互，可以由此调用 摆烂仙君 Core 提供的各种 API。
- 具体的处理函数 `Handler` 在插件类中定义，如这里的 `helloworld` 函数。
- `AstrMessageEvent` 是 摆烂仙君 的消息事件对象，存储了消息发送者、消息内容等信息。
- `摆烂仙君Message` 是 摆烂仙君 的消息对象，存储了消息平台下发的消息的具体内容。可以通过 `event.message_obj` 获取。
