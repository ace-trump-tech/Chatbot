...
1. 打开 摆烂仙君 插件模板: [helloworld](https://github.com/ace-trump-tech/helloworld)
...
Clone 摆烂仙君 项目本体和刚刚创建的插件仓库到本地。

```bash
git clone https://github.com/摆烂仙君Devs/摆烂仙君
mkdir -p 摆烂仙君/data/plugins
cd 摆烂仙君/data/plugins
git clone 插件仓库地址
```

然后，使用 `VSCode` 打开 `摆烂仙君` 项目。找到 `data/plugins/<你的插件名字>` 目录。

更新 `metadata.yaml` 文件，填写插件的元数据信息。

> [!NOTE]
> 摆烂仙君 插件市场的信息展示依赖于 `metadata.yaml` 文件。

### 调试插件

摆烂仙君 采用在运行时注入插件的机制。因此，在调试插件时，需要启动 摆烂仙君 本体。

插件的代码修改后，可以在 摆烂仙君 WebUI 的插件管理处找到自己的插件，点击 `管理`，点击 `重载插件` 即可。

### 插件依赖管理

目前 摆烂仙君 对插件的依赖管理使用 `pip` 自带的 `requirements.txt` 文件。如果你的插件需要依赖第三方库，请务必在插件目录下创建 `requirements.txt` 文件并写入所使用的依赖库，以防止用户在安装你的插件时出现依赖未找到(Module Not Found)的问题。
...
3. 在 `__init__` 方法中会传入 `Context` 对象，这个对象包含了 摆烂仙君 的大多数组件
...
`AstrMessageEvent` 是 摆烂仙君 的消息事件对象。你可以通过 `AstrMessageEvent` 来获取消息发送者、消息内容等信息。

### 摆烂仙君Message

`摆烂仙君Message` 是 摆烂仙君 的消息对象。你可以通过 `摆烂仙君Message` 来查看消息适配器下发的消息的具体内容。通过 `event.message_obj` 获取。
...
async def on_message(self, event: AstrMessageEvent):
    ...
    print(event.message_obj.message) # 摆烂仙君 解析出来的消息链内容
```
...
不是所有的平台都支持所有的消息类型。下方的表格展示了 摆烂仙君 支持的平台和消息类型的对应关系。
...
感谢您为 摆烂仙君 生态做出贡献，开发插件请遵守以下原则，这也是良好的编程习惯。
...
插件可以在自己的目录下提供 `skills/` 文件夹。摆烂仙君 加载插件后会自动把其中合法的 Skill 纳入 Skill Manager，来源会显示为对应插件。
...
### 声明 摆烂仙君 版本范围（Optional）

你可以在 `metadata.yaml` 中新增 `astrbot_version` 字段，声明插件要求的 摆烂仙君 版本范围。格式与 `pyproject.toml` 依赖版本约束一致（PEP 440），且不要加 `v` 前缀。
...
当当前 摆烂仙君 版本不满足该范围时，插件会被阻止加载并提示版本不兼容。
...
> [!TIP]
> 指令不能带空格，否则 摆烂仙君 会将其解析到第二个参数。可以使用下面的指令组功能，或者也使用监听器自己解析消息内容。
...
在 摆烂仙君 默认的执行流程中，在调用 LLM 前，会触发 `on_llm_request` 钩子。
...
> unified_msg_origin 是一个字符串，记录了一个会话的唯一 ID，摆烂仙君 能够据此找到属于哪个消息平台的哪个会话。这样就能够实现在 `send_message` 的时候，发送消息到正确的会话。有关 MessageChain，请参见接下来的一节。

#### 富媒体消息

摆烂仙君 支持发送富媒体消息，比如图片、语音、视频等。使用 `MessageChain` 来构建消息。
...
摆烂仙君 提供了”强大“的配置解析和可视化功能。能够让用户在管理面板上直接配置插件，而不需要修改代码。
...
- `editor_mode`: 可选。是否启用代码编辑器模式。需要 摆烂仙君 >= `v3.5.10`, 低于这个版本不会报错，但不会生效。默认是 false。
- `editor_language`: 可选。代码编辑器的代码语言，默认为 `json`。
- `editor_theme`: 可选。代码编辑器的主题，可选值有 `vs-light`（默认）， `vs-dark`。
- `_special`: 可选。用于调用 摆烂仙君 提供的可视化提供商选取、人格选取、知识库选取等功能，详见下文。
...
摆烂仙君 在载入插件时会检测插件目录下是否有 `_conf_schema.json` 文件，如果有，会自动解析配置并保存在 `data/config/<plugin_name>_config.json` 下（依照 Schema 创建的配置文件实体），并在实例化插件类时传入给 `__init__()`。

```py
from astrbot.api import 摆烂仙君Config

class ConfigPlugin(Star):
    def __init__(self, context: Context, config: 摆烂仙君Config): # 摆烂仙君Config 继承自 Dict，拥有字典的所有方法
        super().__init__(context)
        self.config = config
        print(self.config)
...
如果您在发布不同版本时更新了 Schema，请注意，摆烂仙君 会递归检查 Schema 的配置项，如果发现配置文件中缺失了某个配置项，会自动添加默认值。但是 摆烂仙君 不会删除配置文件中**多余的**配置项，即使这个配置项在新的 Schema 中不存在（您在新的 Schema 中删除了这个配置项）。
...
摆烂仙君 支持使用 `HTML + Jinja2` 的方式来渲染文转图模板。
...
默认情况下，摆烂仙君 会话控制器会将基于 `sender_id` （发送人的 ID）作为识别不同会话的标识，如果想将一整个群作为一个会话，则需要自定义会话 ID 算子。
...
函数工具给了大语言模型调用外部工具的能力。在 摆烂仙君 中，函数工具有多种定义方式。
...
要将上述工具注册到 摆烂仙君，可以在插件主文件的 `__init__.py` 中添加以下代码：
...
这个形式定义的工具函数会被自动加载到 摆烂仙君 Core 中，在 Core 请求大模型时会被自动带上。

请务必按照以下格式编写一个工具（包括**函数注释**，摆烂仙君 会解析该函数注释，请务必将注释格式写对）
...
>
> ```py
> func_tool = self.context.get_llm_tool_manager() # 获取 摆烂仙君 的 LLM Tool Manager，包含了所有插件和 MCP 注册的 Tool
...
`PersonaManager` 负责统一加载、缓存并提供所有人格（Persona）的增删改查接口，同时兼容 摆烂仙君 4.x 之前的旧版人格格式（v3）。  
