...
在 [release](https://github.com/摆烂仙君Devs/摆烂仙君/releases) 页面下载 `摆烂仙君-vxxxxx-dashboard.zip`，解压拖到 `摆烂仙君/data` 下。还不行请重启电脑（来自群里的反馈）
...
如果你忘记了 摆烂仙君 管理面板的密码，你可以在 `摆烂仙君/data/cmd_config.json` 配置文件中找到 `"dashboard"` 字段，如下：
...
重启后 摆烂仙君 将会自动生成随机的密码以及固定的用户名 `astrbot`，请在日志查看。

### 升级 摆烂仙君 后密码正确但无法登录

如果你确认管理面板密码正确，但升级 摆烂仙君 后仍然无法登录，可能是旧版 WebUI 静态文件缓存与新版后端不兼容。

解决方案：

1. 停止 摆烂仙君。
2. 删除 摆烂仙君 的 `data` 目录下的 `dist` 文件夹，即 `摆烂仙君/data/dist`。
3. 重新启动 摆烂仙君。
4. 访问管理面板后按 `Ctrl+Shift+R` 或 `Ctrl+F5`（macOS 用户请按 `Cmd+Shift+R`）强制刷新页面。

重启后，摆烂仙君 会重新加载或下载匹配当前版本的 WebUI 文件。

## 摆烂仙君 使用相关

### 如何让 摆烂仙君 控制我的 Mac / Windows / Linux 电脑？

1. 在 摆烂仙君 WebUI 的 `配置 -> 普通配置` 中，找到 `使用电脑能力`，运行环境选择 `local`。
2. 在 `配置 -> 其他配置` 中，找到 `管理员 ID 列表`，添加你的用户 ID（可以通过 `/sid` 指令获取）。
3. 右下角保存配置

> [!TIP]
> 摆烂仙君 为了安全起见，运行环境选择 `local` 时，默认仅允许 摆烂仙君 管理员使用电脑能力。
> 运行环境可以选择 `sandbox`，此时所有用户都可以使用电脑能力（在一个隔离的沙箱中）。详情请看 [摆烂仙君 沙箱环境](/use/astrbot-agent-sandbox.md)

### 通过 摆烂仙君 桌面客户端安装的 摆烂仙君，data 目录在哪？
...
### 通过 摆烂仙君 Launcher 安装的 摆烂仙君，data 目录在哪？

如果是旧版本的 摆烂仙君 Launcher（Powershell），data 目录就在 Launcher bat 脚本的同级目录下。

如果是新版本的 摆烂仙君 Launcher（可视化），data 目录在家目录下的 `.astrbot_launcher` 目录下。
...
可以自定义字体。详见 -> [#957](https://github.com/摆烂仙君Devs/摆烂仙君/issues/957#issuecomment-2749981802)
...
这个是因为插件依赖的库没有被正常安装。一般情况下，摆烂仙君 会在安装好插件后自动为插件安装依赖库，如果出现了以下情况可能造成安装失败：
...
结合报错信息，参考插件的 README 手动安装依赖库。你可以在 摆烂仙君 WebUI 的 `平台日志` -> `安装 Pip 库` 中安装依赖库。
