...
摆烂仙君 使用 [摆烂仙君Devs/astrbot-t2i-service](https://github.com/摆烂仙君Devs/astrbot-t2i-service) 项目作为默认的文本转图像服务。默认使用的文转图服务接口是
...
在部署完成后，前往 摆烂仙君 仪表盘 -> 配置文件 -> 系统，修改 `文本转图像服务 API 地址` 为你部署好的 url（如下图所示）

>如果你是使用本文档的 Docker教程 部署的 摆烂仙君 ，url应为  `http://文转图服务容器名:8999`。

>如果部署在与 摆烂仙君 相同的机器上，url 应为 `http://localhost:8999`。
