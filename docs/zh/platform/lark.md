...
飞书（Lark）支持两种创建方式：在 摆烂仙君 中扫码一键创建，或在飞书开发者后台手动创建企业自建应用。

### 方式一：扫码一键创建

需要版本 >4.25.0。

进入 摆烂仙君 管理面板，点击左边栏 `机器人`，然后点击 `+ 创建机器人`，选择 `lark(飞书)`。

在 `选择创建方式` 中选择 `扫码一键创建`，按需选择国内版或海外版，然后使用手机飞书扫描页面中的二维码并确认。创建成功后，摆烂仙君 会自动写入该应用的 `app_id`、`app_secret` 和域名配置。
...
>
> App ID 获取方式：回到 摆烂仙君 的 `机器人` 页，找到刚刚创建的飞书机器人，点击 `编辑`，弹出的对话框中可以看到 App ID。
>
> ```text
...
如果您选择了 `webhook` 方式，选择了之后，前往飞书的开发者后台，点击事件与回调，点击加密策略，填写 Encrypt Key。这不是必须的，摆烂仙君 十分注重你的数据安全，所以请务必填写。填写后复制 `Encrypt Key` 和 `Verification Token` 到 摆烂仙君 配置的 `encrypt_key` 和 `verification_token` 处。
...
搜索刚刚创建的机器人的名字。比如教程创建了 `摆烂仙君` 机器人：
