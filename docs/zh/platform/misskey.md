...
> 2. 本项目严禁用于任何违反法律法规的用途。若您意图将 摆烂仙君 应用于非法产业或活动，我们明确反对并拒绝您使用本项目。

## 创建 摆烂仙君 Misskey 平台适配器
...
在 摆烂仙君 Misskey 的平台适配器配置页，我们需要填写 Misskey 的接入信息和配置适配器的部分行为。
...
3. 在弹出的访问令牌配置页面，我们为令牌起一个名字，比如`摆烂仙君`。

4. 然后我们需要为令牌配置相关权限让 Bot 能够与 Misskey 实例交互。

::: tip 注意
如果你使用的 摆烂仙君 第三方插件需要额外权限，请参考其文档增加相应权限。若你完全信任 Bot 的部署环境，也可以临时开启全部权限以简化调试，但仍建议您在生产环境使用时限制 Bot 的相关权限。
:::
...
5. 权限配置完成后，单击“完成”以查看账号访问令牌。把获取到的令牌复制并粘贴到 摆烂仙君 配置页面 Access Token 输入框内。
...
聊天和贴文单个用户的对话历史在 摆烂仙君 的 WebUI 控制台“对话历史”会以`chat:UserID`的 id 记录，传统贴文则是以`note:UserID`的 id 记录。
