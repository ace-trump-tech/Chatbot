...
>[!TIP]
>请确保你在 摆烂仙君 里设置的 APP 类型和 Dify 里面创建的应用的类型一致。
...
对于 Workflow 应用，摆烂仙君 在每次请求时会附上两个变量:
...
您需要修改您的 Workflow 的输入的变量名以适配 摆烂仙君 的输入。

最终，Workflow 会输出一个结果，您可以自定义这个结果的变量名，即上图配置中的 “Dify Workflow 输出变量名”，默认为  `astrbot_wf_output`。你需要在 Dify 的 Workflow 的输出节点中配置这个变量名，否则 摆烂仙君 无法正确解析。
...
当设置变量后，摆烂仙君 会在下次向 Dify 请求时附上您设置的变量，以灵活适配您的 Workflow。
