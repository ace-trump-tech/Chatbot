...
from astrbot.dashboard.server import 摆烂仙君Dashboard
...
@pytest.fixture
def registered_plugin_page(core_lifecycle_td: 摆烂仙君CoreLifecycle, monkeypatch):
    plugin_root = (
...
async def core_lifecycle_td(tmp_path_factory):
    ...
    core_lifecycle = 摆烂仙君CoreLifecycle(log_broker, db)
...
def app(core_lifecycle_td: 摆烂仙君CoreLifecycle):
    """Creates a Quart app instance for testing."""
    shutdown_event = asyncio.Event()
    # The db instance is already part of the core_lifecycle_td
    server = 摆烂仙君Dashboard(core_lifecycle_td, core_lifecycle_td.db, shutdown_event)
    server.app._dashboard_server = server  # expose for test cleanup
    return server.app


def _resolve_dashboard_password(core_lifecycle_td: 摆烂仙君CoreLifecycle) -> str:
...
def test_dashboard_uses_bundled_dist_when_data_dist_is_stale(
    core_lifecycle_td: 摆烂仙君CoreLifecycle,
    monkeypatch,
    tmp_path,
):
...
@pytest_asyncio.fixture(scope="module")
async def authenticated_header(app: Quart, core_lifecycle_td: 摆烂仙君CoreLifecycle):
...
async def test_plugin_page_content_supports_cookie_auth(
    ...
    assert "摆烂仙君PluginPage" in bridge_content
...
async def test_plugin_page_content_issues_scoped_asset_token(
    ...
    assert "window.摆烂仙君PluginPage?.__setInitialContext" in bridge_js
