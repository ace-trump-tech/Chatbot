...
def test_build_skills_prompt_absolute_path_in_example():
    ...
    skills = [
        SkillInfo(
            name="foo",
            description="do foo",
            path="/home/pan/摆烂仙君/skills/foo/SKILL.md",
            active=True,
        ),
    ]
    prompt = build_skills_prompt(skills)
    assert "cat /home/pan/摆烂仙君/skills/foo/SKILL.md" in prompt
...
def test_build_skills_prompt_preserves_windows_absolute_path_in_example(monkeypatch):
    ...
    skills = [
        SkillInfo(
            name="foo",
            description="do foo",
            path="C:/摆烂仙君/data/skills/foo/SKILL.md",
            active=True,
        ),
    ]
    prompt = build_skills_prompt(skills)
    assert 'type "C:/摆烂仙君/data/skills/foo/SKILL.md"' in prompt
...
def test_build_skills_prompt_quotes_windows_paths_with_spaces(monkeypatch):
    ...
    skills = [
        SkillInfo(
            name="foo",
            description="do foo",
            path="C:/摆烂仙君/My Skills/foo/SKILL.md",
            active=True,
        ),
    ]
    prompt = build_skills_prompt(skills)
    assert 'type "C:/摆烂仙君/My Skills/foo/SKILL.md"' in prompt
...
def test_build_skills_prompt_normalizes_windows_backslashes_in_example(monkeypatch):
    ...
    skills = [
        SkillInfo(
            name="foo",
            description="do foo",
            path=r"C:\摆烂仙君\My Skills\foo\SKILL.md",
            active=True,
        ),
    ]
    prompt = build_skills_prompt(skills)
...
def test_build_skills_prompt_preserves_drive_colon_while_sanitizing_unsafe_chars(
    ...
    skills = [
        SkillInfo(
            name="foo",
            description="do foo",
            path="C:/摆烂仙君/data/skills/fo`o/SKILL.md",
            active=True,
        ),
    ]
    prompt = build_skills_prompt(skills)
    ...
    example_fragment = prompt.split("(e.g. `", 1)[1].split("`).", 1)[0]
    assert example_fragment == 'type "C:/摆烂仙君/data/skills/foo/SKILL.md"'
...
def test_list_skills_description_from_sandbox_cache(monkeypatch, tmp_path: Path):
        ...
        [
            {
                ...
                "path": "/home/pan/摆烂仙君/skills/web-scrape/SKILL.md",
            }
        ]
    )
    ...
    assert "/home/pan/摆烂仙君/skills/web-scrape/SKILL.md" in s.path
