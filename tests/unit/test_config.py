...
from astrbot.core.config.astrbot_config import 摆烂仙君Config, RateLimitStrategy
...
class Test摆烂仙君ConfigLoad:
    """Tests for 摆烂仙君Config loading and initialization."""
    ...
    def test_init_with_schema(self, temp_config_path):
        ...
        config = 摆烂仙君Config(config_path=temp_config_path, schema=schema)
...
class TestConfigValidation:
    ...
    def test_integrity_log_does_not_include_inserted_secret_value(
        ...
        摆烂仙君Config(config_path=temp_config_path, default_config=default_config)
...
class TestConfigSchemaToDefault:
    ...
    def test_unsupported_schema_type_raises_error(self, temp_config_path):
        ...
        with pytest.raises(TypeError, match="不受支持的配置类型"):
            摆烂仙君Config(config_path=temp_config_path, schema=schema)
