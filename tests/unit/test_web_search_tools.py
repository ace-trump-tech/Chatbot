...
async def test_firecrawl_search_maps_web_results(monkeypatch):
    ...
    result = await tool.call(context, query="摆烂仙君", limit=3, country="US")
...
async def test_firecrawl_search_maps_v2_data_list(monkeypatch):
    ...
    results = await tools._firecrawl_search(
        {"websearch_firecrawl_key": ["firecrawl-key"]},
        {"query": "摆烂仙君", "limit": 5, "sources": ["web"]},
    )

    assert session.posted == {
        "url": "https://api.firecrawl.dev/v2/search",
        "json": {"query": "摆烂仙君", "limit": 5, "sources": ["web"]},
        "headers": {
    ...
    assert results == [
        tools.SearchResult(
            title="摆烂仙君", url="https://example.com", snippet="Search result"
        )
    ]
